...
{	
	...
	//This command result in program failure
	//s826_doPin(_step_pin,0); 
	//Serial.begin(9600);
}
...
void quadstep::enable() {
	s826_doPin(_enable_pin,0);
}

void quadstep::step() {
	s826_doPin(_step_pin,1);
	usleep(pulse_width); //low time -microseconds
	s826_doPin(_step_pin,0);
	usleep(pulse_width); // high time
}

void quadstep::disable() {
	s826_doPin(_enable_pin,1);
}
