...
double AssignmentProblemSolver::Solve(vector<vector<double> >& DistMatrix,vector<int>& Assignment,TMethod Method)
{
	...
	int N=DistMatrix.size(); // number of tracks
	int M=DistMatrix[0].size(); // number of lines measurements
...
void AssignmentProblemSolver::assignmentoptimal(int *assignment, double *cost, double *distMatrixIn, int nOfRows, int nOfColumns)
{
	...
	*cost = 0;
	...
	// Generate a working copy of the distance matrix
	// and verification of the matrix elements on the positive :)
	...
	// verification and assignment cycle
