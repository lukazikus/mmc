...
static void * autonomy_thread(void * threadid){
    ...
    float angle_des=0, pull_const=0, int_const=0, der_const=0, errorPull=0, errorPullPrev=0, errorSum=0,\
    ...
    /*Set up occupancy grid to all 1s
    ...
    */
    ...
    while ( MMCThread ) {
        ...
        if(state == -1){ // Calibration phase, get arena boundary points and occupancy grid
            ...
            points[0].x = 10; points[0].y = 400;
            points[1].x = 600; points[2].y = 10;
            points[2].x = 350; points[3].y = 400;
            points[3].x = 350; points[4].y = 360;
            ...
            create_og(occ_grid, points, num_points);
            printf("%d\n",occ_grid[400][350]);
        ...
        }else if(autonomous && state == 0){ // Go towards cargo
        ...
        errorPullPrev = errorPull;
        errorPull = sqrt(pow(GOAL_coorX - COM_coorX, 2) + pow(GOAL_coorY - COM_coorY, 2));
        errorSum = errorSum + errorPull;
        errorDiff = errorPull - errorPullPrev;
...
void create_og(int** occ_grid, Point points[], int num_points){
    for(int i=0; i < num_points; i++){
        occ_grid[points[i].y][points[i].x] = 0;
    }
}
